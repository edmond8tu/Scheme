...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not check_procedure(hold):
            return temp.eval_call(rest, env)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        temp = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, temp, env)
        # END PROBLEM 5
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21  
        hold = scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(hold), env)
        # END Problem 21
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    elif expressions.second is nil and scheme_eval(expressions.first, env, True):
        return scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        if bindings.first.second.second is not nil:
            raise SchemeError
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            hold = scheme_optimized_eval(first, env)
            check_procedure(hold)
            result = hold.eval_call(rest, env)
            # END PROBLEM 20
    return result
